...
// Refresh custom CP tab
window.customCPRefresh = async function() {
    const athleteId = window.customCPAthleteId;
    const athlete = window.allAthletes.find(a => a.id === athleteId);
    const tabContent = document.getElementById('athlete-tab-content');
    await renderCustomCPTab(athleteId, athlete, tabContent);
};

window.switchCustomCPPeriod = function(period) {
    window.customCPCurrentPeriod = period;

    let data = null;
    if (period === 'custom') {
        data = window.customCPPeriodData ? window.customCPPeriodData.custom : null;
    } else {
        data = window.customCPPeriodData ? window.customCPPeriodData[period] : null;
    }
    ...
    const info = document.getElementById('custom-cp-period-info');
    if (info) {
        let periodLabel = '90 giorni';
        if (period === 'allTime') periodLabel = 'Tutto il tempo';
        if (period.startsWith('season-')) {
            ...
            periodLabel = season ? season.name : 'Stagione';
        }
        if (period === 'custom') {
            periodLabel = `${window.customCPPeriodStart || '-'} -> ${window.customCPPeriodEnd || '-'}`;
        }
        info.textContent = `Periodo attivo: ${periodLabel}`;
    }
    ...
    const stats = document.getElementById('custom-cp-stats');
    const box = document.getElementById('custom-cp-results-box');
...
window.showCustomCPPeriodPicker = function(athleteId) {
    createModal(
        ...
        <div style="padding: 1.5rem;">
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="date" id="custom-cp-period-start" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
            </div>
            <div style="margin-bottom: 1.5rem;">
                <label style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Data fine:</label>
                <input type="date" id="custom-cp-period-end" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
            </div>
            <div style="display: flex; gap: 1rem;">
                <button class="btn btn-primary" onclick="loadCustomCPPeriod(${athleteId})">Carica dati</button>
    ...
    document.getElementById('custom-cp-period-end').value = today.toISOString().split('T')[0];
    document.getElementById('custom-cp-period-start').value = sixMonthsAgo.toISOString().split('T')[0];
};

window.loadCustomCPPeriod = async function(athleteId) {
    const startDate = document.getElementById('custom-cp-period-start').value;
    const endDate = document.getElementById('custom-cp-period-end').value;
    ...
    if (startDate >= endDate) {
        showToast('Data inizio deve essere prima di data fine', 'warning');
        return;
    }
    ...
    try {
        ...
        const response = await fetch(`/api/athletes/${athleteId}/power-curve?oldest=${startDate}&newest=${endDate}`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }

        const customData = await response.json();
        if (!window.customCPPeriodData) {
            window.customCPPeriodData = {};
        }
        window.customCPPeriodData.custom = customData;
        window.customCPPeriodStart = startDate;
        window.customCPPeriodEnd = endDate;

        const radios = document.querySelectorAll('input[name="custom-cp-period"]');
        radios.forEach(r => { r.checked = false; });

        window.switchCustomCPPeriod('custom');
        showToast(`Caricati dati dal ${formatItalianDate(startDate)} al ${formatItalianDate(endDate)}`, 'success');
    } catch (error) {
        console.error('Error loading custom CP period:', error);
...
function calculateAndRenderCP(allTimes, allPowers, valuesPerWindow, minPercentile, sprintSeconds, useMediumPointFallback = 1, useLongPointFallback = 1) {
    try {
        // Use the unified point selection function from omnipd.js
        const result = selectPointsForPercentile(allTimes, allPowers, minPercentile, valuesPerWindow, sprintSeconds, useMediumPointFallback === 1, useLongPointFallback === 1);
        
        let selectedTimes = result.times;
        let selectedPowers = result.powers;
        let forcedMediumPoint = result.forcedMediumPoint;
        let forcedLongPoint = result.forcedLongPoint;
        ...
        updateCPStats(cpResult, filtered);

        // Create charts
        createCPCharts(allTimes, allPowers, selectedTimes, selectedPowers, cpResult);
...
function calculateAndRenderCPWithAutoSearch(allTimes, allPowers, valuesPerWindow, sprintSeconds, useMediumPointFallback = 1, useLongPointFallback = 1) {
    try {
        ...
        // Auto-search for percentile starting from 100%, decreasing until we have enough points
        while (currentPercentile >= 0) {
            // Use the unified point selection function
            const result = selectPointsForPercentile(allTimes, allPowers, currentPercentile, valuesPerWindow, sprintSeconds, useMediumPointFallback === 1, useLongPointFallback === 1);
