...
function refreshTVTable() {
    ...
    tbody.innerHTML = tvList.map((km, index) => `
        <tr>
            <td>
                <strong style="color: #3b82f6;">${km.toFixed(1)} km</strong>
                <small style="color: #666; margin-left: 10px;">
                    (${((km / (currentRaceData?.distance_km || 1)) * 100).toFixed(1)}% percorso)
                </small>
            </td>
            <td>
...
window.removeTV = function(index) {
    const km = tvList[index];
    tvList.splice(index, 1);
    refreshTVTable();
...
window.clearAllTV = function() {
    ...
    if (confirm(`Rimuovere tutti i ${tvList.length} Traguardi Volanti?`)) {
        tvList = [];
        refreshTVTable();
...
window.addGPM = function() {
    ...
    gpmList.push({ km, category });
    gpmList.sort((a, b) => a.km - b.km); // Sort by km
    refreshGPMTable();
    kmInput.value = '';
    kmInput.focus();
...
function refreshGPMTable() {
    ...
    tbody.innerHTML = gpmList.map((gpm, index) => `
        <tr>
            <td>
                <strong style="color: #10b981;">${gpm.km.toFixed(1)} km</strong>
                <small style="color: #666; margin-left: 10px;">
                    (${((gpm.km / (currentRaceData?.distance_km || 1)) * 100).toFixed(1)}% percorso)
                </small>
            </td>
            <td>
...
window.removeGPM = function(index) {
    const gpm = gpmList[index];
    gpmList.splice(index, 1);
    refreshGPMTable();
...
window.clearAllGPM = function() {
    ...
    if (confirm(`Rimuovere tutti i ${gpmList.length} Gran Premi di Montagna?`)) {
        gpmList = [];
        refreshGPMTable();
...
window.initializeMetricsTab = function() {
    ...
    if (currentRaceData?.tv_points) {
        tvList = [...currentRaceData.tv_points];
        refreshTVTable();
    }
    
    if (currentRaceData?.gpm_points) {
        gpmList = [...currentRaceData.gpm_points];
        refreshGPMTable();
    }
