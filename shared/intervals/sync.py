...
class IntervalsSyncService:
        ...
        try:
            ...
            for activity in activities:
                ...
                try:
                    ...
                    logger.warning(f"Errore scaricamento dettagli {activity_id_str}: {e}")
                    enriched.append(activity)
    ...
    def fetch_athlete_power_settings(self) -> Tuple[Optional[Dict], str]:
        ...
        try:
            ...
            power_data: Dict[str, Optional[float]] = {
            ...
            if isinstance(height_m, (int, float)) and height_m > 0:
                power_data["height_cm"] = float(height_m) * 100.0
            ...
            if sport_settings:
                ...
                ftp_value = cycling_settings.get('ftp')
                if isinstance(ftp_value, (int, float)):
                    power_data["cp"] = float(ftp_value)
                    logger.info(f"   FTP/CP: {power_data['cp']} W")
                
                # W' (W Prime)
                w_prime_value = cycling_settings.get('w_prime')
                if isinstance(w_prime_value, (int, float)):
                    power_data["w_prime"] = float(w_prime_value)
                ...
                if mmp_model:
                    critical_power = mmp_model.get('criticalPower')
                    if isinstance(critical_power, (int, float)):
                        power_data["ecp"] = float(critical_power)
                        logger.info(f"   eCP (estimated): {power_data['ecp']} W")
                    w_prime_est = mmp_model.get('wPrime')
                    if isinstance(w_prime_est, (int, float)):
                        power_data["ew_prime"] = float(w_prime_est)
